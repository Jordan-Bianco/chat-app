...
const rejectRequest = (invitation, index) => {
    Inertia.post(route('group.invitation.reject', { id: invitation.from }), [], {
        onStart: () => { 
...
const acceptRequest = (invitation, index) => {
    Inertia.post(route('group.invitation.accept', { id: invitation.from }), [], {
        onStart: () => { 
            processing.value.index = index;
...
<template>
    <ProfileLayout :user="user">
        <h2 class="mb-8 font-medium text-lg text-white">invitations</h2>
        ...
        <div
            ...
            <div>
                <p>You have been invited to the
                    <Link
                        :href="route('group.show', { slug: invitation.slug })"
            ...
            <div class="flex justify-end space-x-2">
                <button
                    type="button"
                    @click="rejectRequest(invitation, index)"
                ...
                <button
                    @click="acceptRequest(invitation, index)"
