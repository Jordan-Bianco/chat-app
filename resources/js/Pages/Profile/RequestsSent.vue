...
const discardJoinRequest = (id, index) => {
    Inertia.post(route('group.join.store', { id: id }), [], {
        onStart: () => { 
...
<template>
    <ProfileLayout :user="user">
        <h2 class="mb-8 font-medium text-lg text-white">Request sent</h2>
        <div
            ...
            <div>
                <p>You requested to enter
                    <Link
                        :href="route('group.show', { slug: request.slug })"
            ...
            <button
                @click="discardJoinRequest(request.to, index)"
                class="w-24 flex items-center justify-center py-1.5 bg-lime-500 hover:bg-opacity-70 border border-transparent rounded-lg text-xs text-zinc-800 focus:outline-none focus:ring-2 focus:ring-lime-300 transition duration-150">
                    <svg
                        v-if="processing.index === index"
